...
local function decompress(data)
    ...
    do
        ...
        repeat
            ...
            typ = typ == 0 and uncompressed(output, self) or typ == 1 and static(output, self) or
                typ == 2 and dynamic(output, self)
        until last == 1
        local size = #output
        ...
        if os.epoch("utc") > t0 + 1000 then
            os.queueEvent("luagz_yield")
            os.pullEvent("luagz_yield")
            t0 = os.epoch("utc")
        end
    end
    return out
...
local function decompressGZ(data)
    local magic = data:sub(1, 2)
    assert(magic == "\x1F\x8B", "Invalid magic number in GZ data!")
    local start = 11
    local flags = data:byte(4)
    if bit32.btest(flags, 0x04) then start = start + 2 + ("<I2"):unpack(data, 11) end
    if bit32.btest(flags, 0x08) then start = data:find("\0", start) + 1 end
    if bit32.btest(flags, 0x10) then start = data:find("\0", start) + 1 end
    if bit32.btest(flags, 0x02) then start = start + 2 end
    local deflate = data:sub(start, -9)
    return decompress(deflate)
end

return {
    decompress = decompress,
    decompressGZ = decompressGZ
}
